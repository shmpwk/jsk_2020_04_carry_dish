...
class InflateGraspPoint(object):
    def __init__(self, grasp_point):
        self.aug_grasp_point = np.zeros((8, 4))
...
def inferred_point_callback(msg):
    one_shot = False
    while not one_shot:
        data = rospy.wait_for_message('/dish_edge_detector/output_curvature_edge', PointCloud2)
        one_shot = True
...
if __name__ == '__main__':
    ...
    try:
        ...
        rospy.Subscriber('/pointcloud_screenpoint_nodelet/output_point', PointStamped, inferred_point_callback, queue_size=1000)
        #rospy.Subscriber('/dish_edge_detector/output_curvature_edge', PointCloud2, inferred_point_callback, queue_size=1000)
