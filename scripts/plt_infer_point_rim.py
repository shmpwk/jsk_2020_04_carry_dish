...
def update(frame, xx, yy, zz):
    ...
    if frame == 9:
        ...
        VVV2 = Yl + rotated2[1]
        WWW2 = Zl + rotated2[2]
        ax.quiver(Xl, Yl, Zl, UUU/15, VVV/15, WWW/15, color="green", linewidth=5)
       ...
       #ax.scatter(ixl-cy, iyl+cx, cz-izl, s=100, c="blue")
        ...
        VVV2 = YY + rotated2[1]
        WWW2 = ZZ + rotated2[2]
        ax.quiver(XX, YY, ZZ, UUU/15, VVV/15, WWW/15, color="green")
...
grasp_path = "Data/plt_rim/inferred_grasp_point"
g_key = '.pkl'
...
grasp_path = "Data/plt_rim/inferred_point"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
...
grasp_path = "Data/plt_rim/all_edge_point"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
...
grasp_path = "Data/plt_rim/obj_pcl"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
...
grasp_path = "Data/plt_rim/box_pos"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
