...
def update(frame, xx, yy, zz):
    ...
    if frame == 9:
        ...
        ax.scatter(gxl-cy, gyl+cx, cz-gzl, s=100, c="blue")
        ...
        VVV2 = Yl + rotated2[1]
        WWW2 = Zl + rotated2[2]
        ax.quiver(Xl, Yl, Zl, -UUU/25, -VVV/25, -WWW/25, color="green", linewidth=5)
        ax.quiver(Xl, Yl, Zl, -UUU2/6, -VVV2/6, -WWW2/6, color="blue", linewidth=5)
        
       #ax.scatter(ixl-cy, iyl+cx, cz-izl, s=100, c="red")
        plt.pause(0.7)
    elif frame == 10:
        ...
        VVV2 = YY + rotated2[1]
        WWW2 = ZZ + rotated2[2]
        ax.quiver(XX, YY, ZZ, -UUU/25, -VVV/25, -WWW/25, color="green")
        ax.quiver(XX, YY, ZZ, -UUU2/6, -VVV2/6, -WWW2/6, color="blue")
...
grasp_path = "Data/plt_plate/inferred_grasp_point"
g_key = '.pkl'
...
grasp_path = "Data/plt_plate/inferred_point"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
...
grasp_path = "Data/plt_plate/all_edge_point"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
...
grasp_path = "Data/plt_plate/obj_pcl"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
...
grasp_path = "Data/plt_plate/trans"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
...
grasp_path = "Data/plt_plate/box_pos"
g_key = '.pkl'
for g_dir_name, g_sub_dirs, g_files in sorted(os.walk(grasp_path)): 
    for gf in sorted(g_files):
        if g_key == gf[-len(g_key):]:
